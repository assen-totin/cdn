...
ngx_int_t transport_unix(session_t *session, ngx_http_request_t *r) {
	...
	int unix_socket, addr_len_un, bytes_in, auth_response_len, auth_response_pos;
	...
	if (send(unix_socket, session->auth_request, strlen(session->auth_request), 0) == -1) {
	...
	// Await reponse
	auth_response_pos = 0;
	session->auth_response = malloc(UNIX_BUFFER_CHUNK);
	if (session->auth_response == NULL) {
		ngx_log_error(NGX_LOG_EMERG, r->connection->log, 0, "Failed to allocate %l bytes for auth_response.", UNIX_BUFFER_CHUNK);
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}

	auth_response_len = UNIX_BUFFER_CHUNK;
	...
	while(1) {
		...
		if ((bytes_in = read(unix_socket, &msg_in[0], sizeof(msg_in)-1)) == -1) {
			...
			ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Received %u bytes over Unix socket\n", bytes_in);
			if (bytes_in) {
				...
				if (auth_response_pos + bytes_in > auth_response_len - 1) {
					session->auth_response = realloc(session->auth_response, auth_response_len + UNIX_BUFFER_CHUNK);
					if (session->auth_response == NULL) {
						ngx_log_error(NGX_LOG_EMERG, r->connection->log, 0, "Failed to reallocate %l bytes for auth_response.", auth_response_len + UNIX_BUFFER_CHUNK);
						return NGX_HTTP_INTERNAL_SERVER_ERROR;
					}
				}
