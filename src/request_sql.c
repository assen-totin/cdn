 ...
 * Prepare SQL request
 */
ngx_int_t request_sql(session_t *session, metadata_t *metadata, ngx_http_request_t *r) {
	char *query;
	...
	session->sql_query = ngx_pcalloc(r->pool, strlen(query) + strlen(session->auth_value) + 1);
	if (query == NULL) {
